...
def set_montage_chantypes(raw: mne.io.Raw) -> mne.io.Raw:
    """
...
def remove_annots_first_n_trials(raw: mne.io.Raw, n: int) -> mne.io.Raw:
    """
...
def remove_annots_last_n_trials(raw: mne.io.Raw, n: int) -> mne.io.Raw:
    """
    Remove the events for the first N trials.
...
def clean_subjects(sub_list_str: list, paths: PATHS):  # noqa: PLR0915
    """
    ...
    for sub_id in sub_list_str:
        match sub_id:
            case "VR2FEM_S02":
                ...
                fname2 = Path(eeg_data, sub_id + "_2.vhdr")
                raw2 = mne.io.read_raw_brainvision(fname2, preload=True, verbose=False)

                fname3 = Path(eeg_data, sub_id + "_3.vhdr")
                ...
                raw1 = set_montage_chantypes(raw1)

                # I manually checked that the concatenated file is ok, so no checks done here

                # We store as .fif directly
                fname_out = Path(paths.DATA_00_RAWFIF, f"{sub_id}-raw.fif")
            ...
            case "VR2FEM_S03":
                ...
                raw1 = remove_annots_last_n_trials(raw1, 4)
            ...
            case "VR2FEM_S12":
                ...
                fname1 = Path(eeg_data, sub_id + ".vhdr")
                raw1 = mne.io.read_raw_brainvision(fname1, preload=True, verbose=False)
                raw1 = remove_annots_first_n_trials(raw1, 3)
            ...
            case "VR2FEM_S18":
                ...
                fname1 = Path(eeg_data, sub_id + ".vhdr")
                raw1 = mne.io.read_raw_brainvision(fname1, preload=True, verbose=False)
                raw1 = remove_annots_first_n_trials(raw1, 1)
            ...
            case "VR2FEM_S19":
                ...
                raw1 = remove_annots_first_n_trials(raw1, 2)
            ...
            case "VR2FEM_S27":
                ...
                raw1 = mne.io.read_raw_brainvision(fname1, preload=True, verbose=False)
                # delete first 2 events & we lost some events due to bad connection:
                raw1 = remove_annots_first_n_trials(raw1, 4)
            ...
            case "VR2FEM_S26":
                ...
                raw1 = mne.io.read_raw_brainvision(fname1, preload=True, verbose=False)
                # delete first 2 events & we lost some events due to bad connection:
                raw1 = remove_annots_first_n_trials(raw1, 5)
