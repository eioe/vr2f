...
def main(sub_nr: int = None):
    ...
    for subID in sub_list_str:
        ...
        epochs.compute_psd(method="welch", fmin=0, fmax=45, n_fft=nfft, picks=picks, n_jobs=config.N_JOBS).plot()

        # you can select the bad channels here:
        epochs.plot(n_epochs=15, n_channels=64, picks=picks)

        # or give as an input here:
        epochs.info["bads"] += []

        # plot evoked
        (epochs.copy().apply_baseline((-timings.DUR_BL, 0)).average().plot(picks="all"))
