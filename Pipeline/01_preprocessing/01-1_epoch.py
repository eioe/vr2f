...
Crop the raw data into epochs (time-locked to the onset of the face stimuli). We label the epochs with useful names, according to our marker logic (https://www.notion.so/mind-body-emotion/LSL-Marker-coding-in-file-067912a0220d4c4b8fb599d0e4b68fc5). We extract epochs for the main analyses ('erp') and another copy ('ica') which we will use to determine the ICA decomposition. Raw data for ERP epochs is filtered [0.1, 40]Hz, for ICA epochs [1.0, 40]Hz.
...
def extract_epochs(raw_data, events, event_id_, tmin, tmax, l_freq, h_freq,
                   baseline=None, bad_epos=None, n_jobs=1):
...
def check_epo_numbers(subID: str, n_epos: int):
    ...
    """
    if subID in ('VR2FEM_S03', 'VR2FEM_S06', 'VR2FEM_S19', 'VR2FEM_S23',
                 'VR2FEM_S32'):
        expected = 717
    else:
        expected = 720
...
def main():
    ...
    event_info = {'mono/id1/neutral':     111,
                  'mono/id1/happy':       112,
                  'mono/id1/angry':       113,
    ...
    for subID in sub_list_str:
        ...
        ev_dict_stripped = {int(re.findall(r'\d+', ev)[0]): ev_dict_full[ev] 
                            for ev in ev_dict_full if len(re.findall(r'\d+', ev)) > 0}
